...
using Full.Abp.CategoryManagement.Web;
...
[DependsOn(
    typeof(CategoryManagementWebModule),
...
public class CategoryManagementWebHostModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(CategoryManagementWebHostModule).Assembly
            );
        });
    }
    ...
    private void ConfigureMenu(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new CategoryManagementWebHostMenuContributor(configuration));
        });
    }
    ...
    private void ConfigureCache(IConfiguration configuration)
    {
        ...
        {
            options.KeyPrefix = "CategoryManagement:";
        });
    }
    ...
    private void ConfigureAuthentication(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAuthentication(options =>
            ...
            .AddAbpOpenIdConnect("oidc", options =>
            {
                ...
                options.Scope.Add("CategoryManagement");
            });
    }
    ...
    private void ConfigureAutoMapper()
    {
        ...
        {
            options.AddMaps<CategoryManagementWebHostModule>();
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<CategoryManagementWebModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}Full.Abp.CategoryManagement.Web", Path.DirectorySeparatorChar)));
            });
        }
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("CategoryManagement");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "CategoryManagement-Protection-Keys");
        }
    }
